...
(unless avail
  (printf "Compression filter not available~n")
  (exit))

(define filter-info (H5Zget_filter_info FILTER))

(when (or (zero? (bitwise-and filter-info H5Z_FILTER_CONFIG_ENCODE_ENABLED))
          (zero? (bitwise-and filter-info H5Z_FILTER_CONFIG_DECODE_ENABLED)))
  (printf "Compression filter not available for encoding and decoding.\n")
  (exit))
...
|#
(set! dcpl (H5Dget_create_plist dset))
...
|#
(define nelmts 0)
(define filter-type (H5Pget_filter dcpl 0 nelmts 0))
(printf "\nFilter type is: ")
(define filters
  (list (list H5Z_FILTER_DEFLATE "H5Z_FILTER_DEFLATE")
        (list H5Z_FILTER_SHUFFLE "H5Z_FILTER_SHUFFLE")
        (list H5Z_FILTER_FLETCHER32 "H5Z_FILTER_FLETCHER32")
        (list H5Z_FILTER_SZIP "H5Z_FILTER_SZIP")
        (list H5Z_FILTER_NBIT "H5Z_FILTER_NBIT")
        (list H5Z_FILTER_SCALEOFFSET "H5Z_FILTER_SCALEOFFSET")))

(printf "~s~n" (cadr (assoc (cadr (assoc 'filter filter-type)) filters )))
